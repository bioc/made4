...
function(dataset, pos=FALSE,  trans=FALSE){
        ...
        # Extract matrix from SummarizedExperiment, ExpressionSet, marrayRaw,  matrix, data.frame. Ade4 expects a data.frame
        if (!is.data.frame(dataset)) dataset<-asDataFrame(dataset)
 
        if (any(is.na(dataset)))
             stop("data.frame must not contain NA values. Use impute.knn in library(impute), KNNimpute from Troyanskaya et al., 2001 or LSimpute from Bo et al., 2004 to impute missing values\n")
